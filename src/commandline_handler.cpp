...
CommandLineHandler::CommandLineHandler() {
  ...
  QCommandLineOption chartTypeOption(QStringList() << "ct" << ct_name,
                                     "Chart type (e.g. Lines, Boxes, 3DBars)", "chart_type",
                                     "Lines");
  mParser.addOption(chartTypeOption);

  QCommandLineOption chartXOption(QStringList() << "cx" << cx_name, "Chart X-axis (e.g. a1, t2)",
                                  "chart_x", "a1");
  mParser.addOption(chartXOption);

  QCommandLineOption chartYOption(QStringList() << "cy" << cy_name,
                                  "Chart Y-axis (e.g. CPUTime, Bytes, RealMeanTime, ItemsMin)",
                                  "chart_y", "RealTime");
  ...
  QCommandLineOption appendOption(QStringList() << "ap" << fa_name,
                                  "Files to append by renaming (uses ';' as separator)",
                                  "files...");
  mParser.addOption(appendOption);

  QCommandLineOption overwriteOption(QStringList() << "ow" << fo_name,
                                     "Files to append by overwriting (uses ';' as separator)",
                                     "files...");
...
bool CommandLineHandler::process(const QApplication& app) {
  ...
  else if (chartType == "splines")
    plotParams.type = ChartSplineType;
  else if (chartType == "bars")
    plotParams.type = ChartBarType;
  else if (chartType == "hbars")
    plotParams.type = ChartHBarType;
  else if (chartType == "boxes")
    plotParams.type = ChartBoxType;
  else if (chartType == "3dbars")
    plotParams.type = Chart3DBarsType;
  else if (chartType == "3dsurface")
    plotParams.type = Chart3DSurfaceType;
  else {
    plotParams.type = ChartLineType;
  ...
  if (chartX.size() >= 2 && (chartX.startsWith("a") || chartX.startsWith("t"))) {
    if (chartX.startsWith("a"))
      plotParams.xType = PlotArgumentType;
    else
      plotParams.xType = PlotTemplateType;
    // Index
    chartX = chartX.mid(1);
    bool ok;
  ...
  // Y-axis
  if (chartY == "cputime")
    plotParams.yType = CpuTimeType;
  else if (chartY == "realtime")
    plotParams.yType = RealTimeType;
  else if (chartY == "iterations")
    plotParams.yType = IterationsType;
  else if (chartY == "bytes" && bchResults.meta.hasBytesSec)
    plotParams.yType = BytesType;
  else if (chartY == "items" && bchResults.meta.hasItemsSec)
    plotParams.yType = ItemsType;

  else if (bchResults.meta.hasAggregate) {
    if (chartY == "cpumintime")
      plotParams.yType = CpuTimeMinType;
    else if (chartY == "cpumeantime")
      plotParams.yType = CpuTimeMeanType;
    else if (chartY == "cpumediantime")
      plotParams.yType = CpuTimeMedianType;
    else if (chartY == "realmintime")
      plotParams.yType = RealTimeMinType;
    else if (chartY == "realmeantime")
      plotParams.yType = RealTimeMeanType;
    else if (chartY == "realmediantime")
      plotParams.yType = RealTimeMedianType;
    else if (bchResults.meta.hasBytesSec) {
      if (chartY == "bytesmin")
        plotParams.yType = BytesMinType;
      else if (chartY == "bytesmean")
        plotParams.yType = BytesMeanType;
      else if (chartY == "bytesmedian")
        plotParams.yType = BytesMedianType;
    } else if (bchResults.meta.hasItemsSec) {
      if (chartY == "itemsmin")
        plotParams.yType = ItemsMinType;
      else if (chartY == "itemsmean")
        plotParams.yType = ItemsMeanType;
      else if (chartY == "itemsmedian")
        plotParams.yType = ItemsMedianType;
    } else {
      plotParams.yType = RealTimeType;
  ...
  if (chartZ.size() >= 2 &&
    ...
    if (chartZ == "auto") {
      ...
      if (chartZ.startsWith("a"))
        plotParams.zType = PlotArgumentType;
      else
        plotParams.zType = PlotTemplateType;
      // Index
      chartZ = chartZ.mid(1);
      bool ok;
      ...
      if ((plotParams.zType == PlotArgumentType &&
           plotParams.zIdx >= bchResults.meta.maxArguments) ||
          (plotParams.zType == PlotTemplateType &&
           plotParams.zIdx >= bchResults.meta.maxTemplates)) {
  ...
  switch (plotParams.type) {
    ...
    case ChartSplineType: {
      PlotterLineChart* plotLines =
          new PlotterLineChart(bchResults, bchIdxs, plotParams, fileName, addFilenames);
      plotLines->show();
      break;
    }
    case ChartBarType:
    case ChartHBarType: {
      PlotterBarChart* plotBars =
          new PlotterBarChart(bchResults, bchIdxs, plotParams, fileName, addFilenames);
      plotBars->show();
      break;
    }
    case ChartBoxType: {
      PlotterBoxChart* plotBoxes =
          new PlotterBoxChart(bchResults, bchIdxs, plotParams, fileName, addFilenames);
      plotBoxes->show();
      break;
    }
    case Chart3DBarsType: {
      Plotter3DBars* plot3DBars =
          new Plotter3DBars(bchResults, bchIdxs, plotParams, fileName, addFilenames);
      plot3DBars->show();
      break;
    }
    case Chart3DSurfaceType: {
      Plotter3DSurface* plot3DSurface =
          new Plotter3DSurface(bchResults, bchIdxs, plotParams, fileName, addFilenames);
      plot3DSurface->show();
      break;
    }
  }

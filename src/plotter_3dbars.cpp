...
void Plotter3DBars::connectUI() {
  ...
  connect(ui->comboBoxTheme, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &Plotter3DBars::onComboThemeChanged);

  // Bars
  setupGradients();
  connect(ui->comboBoxGradient, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &Plotter3DBars::onComboGradientChanged);

  connect(ui->doubleSpinBoxThickness, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &Plotter3DBars::onSpinThicknessChanged);
  connect(ui->doubleSpinBoxFloor, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &Plotter3DBars::onSpinFloorChanged);
  connect(ui->doubleSpinBoxSpacingX, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &Plotter3DBars::onSpinSpaceXChanged);
  connect(ui->doubleSpinBoxSpacingZ, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &Plotter3DBars::onSpinSpaceZChanged);
  ...
  if (!isYTimeBased(mPlotParams.yType))
    ...
    connect(ui->comboBoxTimeUnit, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &Plotter3DBars::onComboTimeUnitChanged);
  }
  ...
  connect(ui->comboBoxAxis, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &Plotter3DBars::onComboAxisChanged);
  ...
  connect(ui->spinBoxLogBase, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &Plotter3DBars::onSpinLogBaseChanged);
  ...
  connect(ui->checkBoxAutoReload, &QCheckBox::stateChanged, this,
          &Plotter3DBars::onCheckAutoReload);
...
void Plotter3DBars::setupChart(const BenchResults& bchResults, const QVector<int>& bchIdxs,
  ...
  if (!hasZParam) {
    ...
    for (const auto& bchSubset : qAsConst(bchSubsets)) {
      ...
      firstCol = false;
      //            qDebug() << "[Multi-NoZ] colLabels:" << colLabels << "|" <<
      //            series->dataProxy()->columnLabels();
    }
    // Add series
  ...
  // Z-param -> one series per benchmark, one row per Z, one column per X
    ...
    for (const auto& bchName : bchNames) {
      ...
      for (const auto& bchZSub : qAsConst(bchZSubs)) {
        ...
        //                qDebug() << "[Multi-Z] curColLabels:" << curColLabels << "|" <<
        //                prevColLabels;
      }
    ...
    if (!bars->seriesList().isEmpty() &&
        bars->seriesList().constFirst()->dataProxy()->rowCount() > 0) {
...
void Plotter3DBars::loadConfig(bool init) {
  ...
  if (auto value = settings.value("bars/gradient"); value.isValid())
    ui->comboBoxGradient->setCurrentText(value.toString());
  if (auto value = settings.value("bars/thick"); value.isValid())
    ui->doubleSpinBoxThickness->setValue(value.toDouble());
  if (auto value = settings.value("bars/floor"); value.isValid())
    ui->doubleSpinBoxFloor->setValue(value.toDouble());
  if (auto value = settings.value("bars/spacing/x"); value.isValid())
    ui->doubleSpinBoxSpacingX->setValue(value.toDouble());
  if (auto value = settings.value("bars/spacing/z"); value.isValid())
    ui->doubleSpinBoxSpacingZ->setValue(value.toDouble());
  ...
  for (int i = 0; i < mAxesParams.size(); ++i) {
    ...
    } else  // y-axis
    {
      ...
      if (auto value = settings.value(prefix + "/ticks", 1); value.isValid())
        ui->spinBoxTicks->setValue(value.toInt());
      if (auto value = settings.value(prefix + "/mticks", 1); value.isValid())
...
void Plotter3DBars::saveConfig() {
  ...
  settings.setValue("bars/gradient", ui->comboBoxGradient->currentText());
  settings.setValue("bars/thick", ui->doubleSpinBoxThickness->value());
  settings.setValue("bars/floor", ui->doubleSpinBoxFloor->value());
  settings.setValue("bars/spacing/x", ui->doubleSpinBoxSpacingX->value());
  settings.setValue("bars/spacing/z", ui->doubleSpinBoxSpacingZ->value());
  ...
  for (int i = 0; i < mAxesParams.size(); ++i) {
    ...
    if (i == 0 || i == 2)  // x-axis or z-axis
    {
      if (axis.minIdx >= 0 && axis.minIdx < axis.range.size() && axis.maxIdx >= 0 &&
          axis.maxIdx < axis.range.size()) {
        settings.setValue(prefix + "/min", axis.range[axis.minIdx]);
        settings.setValue(prefix + "/max", axis.range[axis.maxIdx]);
      }
    } else  // y-axis
    {
...
void Plotter3DBars::onSpinThicknessChanged(double d) { mBars->setBarThickness(d); }

void Plotter3DBars::onSpinFloorChanged(double d) { mBars->setFloorLevel(d); }
  ...
  for (auto& series : chartSeries) {
    ...
    for (int iR = 0; iR < dataProxy->rowCount(); ++iR) {
      ...
      for (int iC = 0; iC < row->size(); ++iC) {
        ...
        dataProxy->setItem(iR, iC, QBarDataItem(static_cast<float>(item->value() * updateFactor)));
      }
    }
  }
...
void Plotter3DBars::onCheckAxisRotate(int state) {
  ...
  if (iAxis == 0)
    axis = mBars->columnAxis();
  else if (iAxis == 1)
    axis = mBars->valueAxis();
  else
    axis = mBars->rowAxis();
    ...
    if (!hasZParam) {
      ...
      if (errorMsg.isEmpty()) {
        ...
        for (const auto& bchSubset : qAsConst(newBchSubsets)) {
          ...
          for (int idx : bchSubset.idxs) {
            // Update item
            oldDataProxy->setItem(
                newRowsIdx, newColsIdx,
                QBarDataItem(static_cast<float>(
                    getYPlotValue(newBchResults.benchmarks[idx], mPlotParams.yType) *
                    mCurrentTimeFactor)));
            ++newColsIdx;
          }
          ++newRowsIdx;

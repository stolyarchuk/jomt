...
struct BenchResults {
  ...
  static QString extractData(const BenchData& data, int argIdx, int tpltIdx,
                             const QString& glyph = "", int argIdx2 = -1, int tpltIdx2 = -1,
                             const QString& glyph2 = "");
  // Replace argument with glyph in BenchData name
  static inline QString extractArgument(const BenchData& data, int argIdx,
                                        const QString& argGlyph) {
    return extractData(data, argIdx, -1, argGlyph);
  }
  // Replace template with glyph in BenchData name
  static inline QString extractTemplate(const BenchData& data, int tpltIdx,
                                        const QString& tpltGlyph) {
  ...
  // Convert parameter name to value (check special names, use incremented fallback if all else
  // fail)
  static double getParamValue(const QString& name, QString& custDataName, bool& custDataAxis,
                              double& fallbackIdx);
  ...
  QVector<BenchSubset> segment2DNames(const QVector<int>& subset, bool isArg1, int idx1,
  ...
  QVector<BenchSubset> groupArgument(const QVector<int>& subset, int argIdx,
                                     const QString& argGlyph) const;
  // Each Benchmark from vector in its own subset % Template
  QVector<BenchSubset> groupTemplate(const QVector<int>& subset, int tpltIdx,
                                     const QString& tpltGlyph) const;
  // Each Benchmark from vector in its own subset % Argument/Template
  QVector<BenchSubset> groupParam(bool isArgument, const QVector<int>& subset, int idx,
